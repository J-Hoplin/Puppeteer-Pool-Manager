...
describe('PriorityQueue function test', () => {
  ...
  describe('enqueue', () => {
    it('should add elements with priority', () => {
      const id1 = queue.enqueue({ payload: 1, priority: 2 });
      const id2 = queue.enqueue({ payload: 2, priority: 1 });
    ...
    it('should maintain priority order', () => {
      queue.enqueue({ payload: 1, priority: 2 });
      queue.enqueue({ payload: 2, priority: 3 });
      queue.enqueue({ payload: 3, priority: 4 });
      queue.enqueue({ payload: 4, priority: 1 });
      ...
      expect(first.payload).toBe(3);
      expect(second.payload).toBe(2);
      expect(third.payload).toBe(1);
      expect(fourth.payload).toBe(4);
    });
  ...
  describe('dequeue', () => {
    it('should return highest priority element first', () => {
      queue.enqueue({ payload: 1, priority: 1 });
      queue.enqueue({ payload: 2, priority: 3 });
      queue.enqueue({ payload: 3, priority: 2 });
  ...
  describe('remove', () => {
    it('should remove element with id', () => {
      const id1 = queue.enqueue({ payload: 1, priority: 1 });
      const id2 = queue.enqueue({ payload: 2, priority: 2 });
  ...
  describe('clear', () => {
    it('should remove all elements', () => {
      queue.enqueue({ payload: 1, priority: 1 });
      queue.enqueue({ payload: 2, priority: 2 });
  ...
  describe('values', () => {
    it('should return all elements with priority order', () => {
      ...
      expect(values[0].payload).toBe(2);
      expect(values[1].payload).toBe(3);
      expect(values[2].payload).toBe(1);
    });
  });
});
